<template>
  <section class="editor">
    ...
    <div :class="view">
      ...
      <button class="toggle" @click="editToggle">
          <i class="fa fa-chevron-left fa-fw" v-show="view==='edit'"></i>
          <i class="fa fa-chevron-right fa-fw" v-show="view==='inspect'"></i>
      </button>
      <article v-html="input | marked"></article>
    </div>
    <div class="panel">
  ...
  export default {
    ...
    created() {
      if (this.$route.query.id) {
        ...
        this.$http.get('/article?id=' + id).then((response)=> {
    ...
    methods: {
      send() {
        ...
        this.$http.post('/save', this.$data).then(()=> {
          this.pop({
            pop: true,
            content: '保存成功',
            cb1: function() {
    ...
    computed: {
      dateStr: {
        set(value) {
          ...
          if (reg.test(value)) {
            let date = RegExp.$1 + '/' + RegExp.$2 + '/' + RegExp.$3
